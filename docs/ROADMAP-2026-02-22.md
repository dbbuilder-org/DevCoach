...
Replace the current pattern (raw PAT sent on every request, stored in sessionStorage) with a proper token exchange so the PAT is transmitted once and never stored client-side.

| ID | SP | Title | Notes |
|----|-----|-------|-------|
| AU-01 | 2 | `POST /auth/login` endpoint | Accepts `{ pat, anthropic_key }`, validates PAT with GitHub (`GET /user`), returns signed JWT (30-day expiry) + `github_username`. Reject with 401 on invalid PAT. |
| AU-02 | 1 | JWT middleware | Replace current PAT-based `get_current_user` dependency with JWT verification using `python-jose`. Keep PAT validation as the issuance path only. |
| AU-03 | 1 | Stop persisting PAT in DB | Remove Fernet-encrypted PAT column from users table (migration). Backend no longer stores PAT at all — GitHub calls use the token carried per-request only during login. |
| AU-04 | 2 | Web: swap PAT for JWT on login | `ConfigScreen` sends PAT once to `/auth/login`, stores returned JWT in `sessionStorage` (not the raw PAT). All subsequent `ApiClient` requests send `Authorization: Bearer <jwt>`. |
| AU-05 | 1 | Anthropic key: keep as per-request header | `X-Anthropic-Key` stays in sessionStorage, sent on every chat/puzzle request. Never stored server-side. Add note in ConfigScreen explaining this. |
| AU-06 | 1 | `POST /auth/refresh` endpoint | Accepts valid JWT, issues a new one (sliding window). Called by web app when JWT is within 7 days of expiry. |

**Flow after JWT buildout:**
```
User enters PAT + Anthropic key (once)
  → POST /auth/login { pat, anthropic_key }
  → Backend: validate PAT with GitHub, issue JWT
  ← { jwt, expires_at, github_username }
  → Store JWT in sessionStorage (not PAT)
  → All requests: Authorization: Bearer <jwt>
                  X-Anthropic-Key: <key>   ← still per-request
  → On tab close: both cleared from sessionStorage
```
...
|-------|-------|-----------------|
| Phase A | JWT auth flow (AU-01–AU-06) — PAT never stored client-side | 1 week |
| Phase B | Multi-session-per-repo (WA-01) + PAT scope validation (EX-02) | 1 week |
| Phase C | VS Code Marketplace publish + onboarding wizard | 1 week |
| Phase D | Team mode — coaching profile aggregation across multiple devs on same repo | 3 weeks |
| Phase E | Slack/GitHub Discussions integration for annotation reminders | 2 weeks |
